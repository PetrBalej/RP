...
# ndop.fs$bgRatioMulti <- c(0.01, 0.05, 0.1, 0.2, 0.3, 0.5)
ndop.fs$bgRatioMulti <- c(0.1)
...
#     cnames.c <- c(cnames[str_detect(cnames, paste0(sp, "$"))], "DRUH", "geometry", "nc_TO.w", "nc_TS.w", "nc_TO", "nc_TS", "nc_AUTOR", "nc_uid", "nc_cellNumber") # paste0("nc_sso_", sp)
...
repeat {
    ...
    for (bgN in ndop.fs$bgRatioMulti) {


        ### nové null ze sample()?
        null.default <- sample(predictors[[1]], bgN)
        ...
        # tgobV$p %<>% group_by(nc_AUTOR, nc_cellNumber, DRUH) %>% slice_head(n = 1) # per pixel


#         p.temp <- ag %>%  filter(!is.na(`nc_TGOB.sso.w3_Accipiter gentilis`))
#         vn <- "nc_TGOB.sso.w3_Accipiter gentilis"
#         agc <- tgbg::bg(p.temp %>% dplyr::select(geometry), predictors[[1]], sigma = ndop.fs$adjusts, weights = p.temp[[paste0(vn)]], output = ndop.fs$outputBgBr, anisotropic = TRUE)


#             ag2 <-  ag %>% group_by(nc_AUTOR, nc_cellNumber, DRUH) %>% slice_head(n = 1) # per pixel
#         p.temp2 <- ag2 %>%  filter(!is.na(`nc_TGOB.sso.w3_Accipiter gentilis`))
#         vn <- "nc_TGOB.sso.w3_Accipiter gentilis"
#         agc2 <- tgbg::bg(p.temp2 %>% dplyr::select(geometry), predictors[[1]], sigma = ndop.fs$adjusts, weights = p.temp2[[paste0(vn)]], output = ndop.fs$outputBgBr, anisotropic = TRUE)


#         agc2 <- agc2


# agAUTOR <- ag %>% group_by(nc_AUTOR) %>%  slice_head(n = 1) # per pixel

# agAUTOR2 <- ag %>% group_by(nc_AUTOR)  %>% summarise( count = n_distinct(nc_uid))

# agAUTOR.j <- as_tibble(agAUTOR) %>% left_join(agAUTOR2, by="nc_AUTOR")
...
p.temp %<>% filter(!!sym(vn) > 0.0 & !!sym(vn) < 1.0) # odstranit extrémy


                collector[[vns]] <- tgbg::bg(p.temp %>% dplyr::select(geometry), predictors[[1]], n = bgN, sigma = ndop.fs$adjusts, weights = p.temp[[paste0(vn)]], output = ndop.fs$outputBgBr, anisotropic = TRUE)
                if (vns == "TS.w" | vns == "TO.w" | vns == "TS.AO.w" | vns == "TO.AO.w") {
                    ...
                    collector[[paste0(vns, ".sso")]] <- tgbg::bg(p.temp.temp %>% dplyr::select(geometry), predictors[[1]], n = bgN, sigma = ndop.fs$adjusts, weights = p.temp.temp[[paste0(vn)]], output = ndop.fs$outputBgBr, anisotropic = TRUE)

                    # + zkombinovat i w druhu a w sso???? TS.w.sso.w?
                }
            } else {
                collector[[vns]] <- tgbg::bg(p.temp %>% dplyr::select(geometry), predictors[[1]], n = bgN, sigma = ndop.fs$adjusts, output = ndop.fs$outputBgBr, anisotropic = TRUE)

                if (vn == "TGOB") {
                    # zkresluju podle druhu
                    collector[[paste0(vns, ".bss")]] <- tgbg::bg(p.temp.bss %>% dplyr::select(geometry), predictors[[1]], n = bgN, sigma = ndop.fs$adjusts, output = ndop.fs$outputBgBr, anisotropic = TRUE)
                }
            }
            ...
            if (vn == "TGOB" | vn == "TS" | vn == "TO" | vn == "TS.AO" | vn == "TO.AO") {
                ...
                collector[[paste0(vns, ".sso")]] <- tgbg::bg(p.temp, predictors[[1]], n = bgN, sigma = ndop.fs$adjusts, output = ndop.fs$outputBgBr, anisotropic = TRUE)
            }
        ...
        saveRDS(e.mx.all, paste0(path.tgob, "23ssos_", druh, "_", rep, "_", bgN, ".rds"))
        saveRDS(out.t, paste0(path.tgob, "t_23ssos_", druh, "_", rep, "_", bgN, ".rds"))
        gc()
        # } # for DRUH
    }
}
