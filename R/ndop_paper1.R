...
SPH_OKRES.source <- st_read(paste0(path.data, "cuzk/SPH_SHP_WGS84/WGS84/SPH_OKRES.shp"))

vzchu.source <- st_read(paste0(path.data, "aopk/vzchu/vzchu.shp"))
...
st_crs(SPH_OKRES.source) <- 4326
st_crs(vzchu.source) <- 4326
...
lsd.path <- paste0(path.prep, "../dataPrep/lsd/", "sitmap_2rad.czechia.ow.rds")
lsd.polygons <- NA
if (file.exists(lsd.path)) {
    lsd.polygons <- readRDS(lsd.path) %>% dplyr::select(geometry)
}
...
SPH_OKRES.source.s <- st_simplify(SPH_OKRES.source, preserveTopology = FALSE, dTolerance = 0.001)

vzchu.source.s <- st_simplify(vzchu.source, preserveTopology = FALSE, dTolerance = 0.001)



# Praha + Ostrava
SPH_OKRES.source.s.selected <- SPH_OKRES.source.s %>%
    filter(ID %in% c(9999, 3807)) %>%
    dplyr::select(geometry)

# Krkonoše + Šumava
vzchu.source.s.selected <- vzchu.source.s %>%
    filter(OBJECTID %in% c(5800, 6153)) %>%
    dplyr::select(geometry)

SPH_STAT.source.s.selected <- SPH_STAT.source.s %>% dplyr::select(geometry)

SPH_OKRES.source.s.selected <- SPH_OKRES.source.s.selected %>% dplyr::select(geometry)
...
ggsave(paste0(path.ndop, "map-species-per-POLE.png"), width = 800, height = 600, units = "px")
...
ggplot() +
    ...
    geom_sf(data = SPH_OKRES.source.s.selected, fill = NA, color = "red", lwd = 0.2) +
    geom_sf(data = vzchu.source.s.selected, fill = NA, color = "#00b900", lwd = 0.2)
ggsave(paste0(path.ndop, "map-occs-per-POLE.png"), width = 800, height = 600, units = "px")
